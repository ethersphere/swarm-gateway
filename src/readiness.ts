...
export async function checkReadiness(bee: Bee, strict: boolean): Promise<boolean> {
  try {
    const readiness = await bee.status.getReadiness({ timeout: READINESS_TIMEOUT_MS })
    const isReady = readiness.status === 'ready'
    if (!strict) {
      return isReady
    }
    const topology = await bee.connectivity.getTopology({ timeout: READINESS_TIMEOUT_MS })
